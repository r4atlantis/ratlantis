#' execute_atlantis2 function
#'
#' This function calls the terminal and executes an Atlantis model.  THIS IS A
#' TEMPORARY FIX TO ALLOW MODELS WITH ECON PARAMETERS.
...
#' location as the atlantis exe file)
#' @param econ_prm name of econ file (needs to be in same
#' location as the atlantis exe file)
...
execute_atlantis2 <- function (atlantis_location, atlantis_exe, biology_nc, output_file_nc,
  ...
  shell(paste("cd ", atlantis_location, " && ", atlantis_exe, " -i ", biology_nc," 0 -o ",
              ...
              physics_prm, " -b ", biology_prm, " -h  ", harvest_prm, " -e ", econ_prm, " -s ", functional_group_csv,
